...
document.getElementById('productsTableContainer').innerHTML = `<table><thead><tr><th>صورة</th><th>المنتج</th><th>البراند</th><th>السعر</th><th>التصنيف</th><th>التوفر</th><th>إجراءات</th></tr></thead><tbody>${
  allProducts.map(p => `<tr>
    ...
    <td><button class="avail-toggle" onclick="toggleAvailability('${p.id}')">${p.available?'🟢':'🔴'}</button> <span style="font-size:11px;color:var(--muted)">${p.available?'متوفر':'نفد'}</span></td>
    <td><div class="action-btns"><button class="action-btn btn-view" onclick="openProductModal('${p.id}')">✏️ تعديل</button><button class="action-btn btn-cancel" onclick="deleteProduct('${p.id}')">🗑</button></div></td>
  </tr>`).join('')
}</tbody></table>`;
  ...
  <div class="product-card-m">
    ...
    <div class="product-card-m-info">
      ...
      <div class="product-card-m-actions">
        <button class="action-btn btn-view" onclick="openProductModal('${p.id}')">✏️ تعديل</button>
        <button class="action-btn btn-cancel" onclick="deleteProduct('${p.id}')">🗑 حذف</button>
      </div>
    </div>
  </div>`).join('');
```
...
if (!nameAr || !nameEn || !brand || !price) {
  msgEl.style.color = 'var(--red)'; msgEl.textContent = '⚠️ يرجى ملء جميع الحقول المطلوبة'; return;
}
if (!allProducts || allProducts.length === 0) allProducts = await getProducts();
if (editingProductId) {
  ...
  if (idx !== -1) {
    ...
    msgEl.style.color = 'var(--red)'; msgEl.textContent = '⚠️ خطأ: المنتج غير موجود'; return;
  }
} else {
  allProducts.push({ id: String(Date.now()), nameAr, nameEn, brand, price, cat, img, badge, available });
}
msgEl.style.color = 'var(--gold)'; msgEl.textContent = '⏳ جاري الحفظ...';
await saveProducts(allProducts);
msgEl.style.color = 'var(--green)'; msgEl.textContent = '✅ تم الحفظ بنجاح!';
